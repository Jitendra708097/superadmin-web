...
export default function Sidebar() {
  ...
  return (
    <aside
      ...
      `}
    >
      <div className={`flex items-center border-b border-[#1e1e35] h-14 flex-shrink-0 ${collapsed ? 'justify-center px-3' : 'px-4 gap-3'}`}>
        <div className="relative flex-shrink-0">
          ...
          <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-[#00d4ff] animate-ping opacity-60" />
          <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-[#00d4ff]" />
        </div>
        {!collapsed && (
          <div className="min-w-0">
            <div className="text-[#e8e8f0] text-sm font-['JetBrains_Mono'] font-bold tracking-tight leading-none">AttendEase</div>
            <div className="text-[#6b6b8a] text-[9px] uppercase tracking-[0.15em] mt-0.5">Super Admin</div>
          </div>
        )}
      </div>
            ...
            <Tooltip key={path} title={collapsed ? label : ''} placement="right">
              <NavLink
                ...
                className={({ isActive }) => `
                  ...
                  ${isActive ? 'bg-[#00d4ff]/10 text-[#00d4ff] border-l-2 border-[#00d4ff]' : 'text-[#6b6b8a] hover:text-[#e8e8f0] hover:bg-[#161625] border-l-2 border-transparent'}
