...
export default function RevenuePage() {
  ...
  const invoiceCols = [
    ...
    { title: 'Amount', dataIndex: 'amount', width: 110, align: 'right', render: (v) => <span className="font-['JetBrains_Mono'] text-xs text-[#ffaa00]">{formatINR(v)}</span> },
    {
      ...
      render: (v) => <span className={`text-[10px] font-['JetBrains_Mono'] uppercase ${v === 'paid' ? 'text-[#00ff88]' : 'text-[#ff3366]'}`}>{v}</span>,
    },
  ];

  return (
    <div className="animate-fade-in">
      <PageHeader title="Billing" subtitle="Platform-wide invoices, subscriptions, and MRR" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#0f0f1a] border border-[#1e1e35] rounded-lg p-5">
          <div className="mb-4">
            <h3 className="text-[#e8e8f0] text-sm font-sans font-medium">Billing by Plan</h3>
          ...
          {planLoading ? (
            ...
            <DarkPieChart data={pieData} colors={pieColors} height={210} tooltipFormatter={(v, name) => [formatINR(v), name]} />
          )}
        </div>
      </div>
      ...
      <div className="bg-[#0f0f1a] border border-[#1e1e35] rounded-lg overflow-hidden">
        ...
        <Table columns={invoiceCols} dataSource={invoices} rowKey="id" size="small" loading={invoiceLoading} pagination={{ pageSize: 20, size: 'small' }} />
      </div>
    </div>
  );
}
